...
class ObjectNullablePref<T>(
    ...
    override fun setToPreference(property: KProperty<*>, value: T?, preference: SharedPreferences) {
        preference.edit().putString(key(property), value?.let { json.writeValueAsString(it) })
    ...
    companion object {
        ...
        fun <T> objectNullablePref(
            clazz: Class<T>,
            defaultSupplier: () -> T? = { null },
            key: String? = null,
            commitByDefault: Boolean = KotPref.commitAllPropertiesByDefault
        ): ReadWriteProperty<KotprefModel, T?> =
            ObjectNullablePref(defaultSupplier, key, commitByDefault, clazz)
    }
}